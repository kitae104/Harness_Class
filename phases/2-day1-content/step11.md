...
- `/docs/OPEN_QUESTIONS.md` — OQ-03 해결 내용
- `/docs/ADR.md` — CD-06, **CD-17**, CD-21
...
3. 체크리스트: 답변 단계에 기대 답변이 없는가 / 채점이 Project 밖 임시 채팅인가 / 사람 표본 3문항이 있는가 / OQ-03 해결 내용이 반영되었는가 / 예상 결과·잘못된 결과 예시 / status draft까지.
...
5. 결과에 따라 `phases/2-day1-content/index.json`의 step 11을 업데이트한다:
   - 성공 → `"status": "completed"`, `"summary": "만든 파일, 반영한 OQ-03 해결 내용, 카드 2장"`
   - 수정 3회 시도 후에도 실패, 또는 D1-07과 어긋남 → `"status": "error"`, `"error_message": "구체적 내용"`
   - OQ-03 open 등 사용자 개입 필요 → `"status": "blocked"`, `"blocked_reason": "구체적 사유"` 후 즉시 중단

## 금지사항

- OQ-03이 open인데 06.mdx를 만들거나 status를 draft로 올리지 마라. 이유: PRACTICE_DESIGN 3절.
