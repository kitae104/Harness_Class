...
1. `content/day1/05.mdx`(`lesson_id: d1-evalset`) — 기준 예시 구조를 따른다.
   ...
   - 제공 20문항은 `<KitDownload id="day1-civil" />`로 안내하고, **질문 파일과 기대 답변 파일의 용도 차이**(답변 단계에는 질문만, 기대 답변은 채점 단계에서만 — CD-17)를 이 교시에서 미리 알린다. 수강생이 **Project 파일 목록에 규정·FAQ 두 파일만 있는지** 확인하는 체크 항목을 넣는다(기대 답변 파일이 Project에 올라가 있으면 D1-06 점수가 부푼다, TOOL_GUIDE 3절).
   - 기대 답변의 "참고(감점 사유 아님)" 표기(자료에 없음 문항의 연락처 안내, 18번 채점 참고)를 채점 기준 설명과 어긋나지 않게 다룬다.
