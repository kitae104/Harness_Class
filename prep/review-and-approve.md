...
AI는 콘텐츠를 draft까지만 올립니다. reviewed는 강사가 직접 `npm run review:approve`로만 올립니다(ADR-011). 이 PC에서는 Smart App Control 때문에 npm이 Windows에서 동작하지 않으므로 **WSL에서** 실행합니다.

## 1. 재검토
- 사이트를 교시 순서대로 수강생 입장에서 읽습니다(로컬: WSL에서 `npm run build && npm run preview`, 또는 PR의 Vercel Preview).
...
## 2. 승인 (WSL)
```
cd /mnt/d/Githubs/Harness_WS/Harness_Class
git status --short        # 파일 전체가 바뀐 것처럼 보이면(줄바꿈) 멈추고 확인
npm run review:approve <id>   # 검토를 마친 교시·카드·키트마다
```
- 아직 draft인 항목과 해시가 깨진 항목은 `python3 scripts/validate_course.py --production`의 V-REV-001·V-REV-002 오류로 한 번에 볼 수 있습니다.
- 카드의 `tested_at`을 채우거나 본문을 고치면 해시가 바뀌므로, **편집을 모두 끝낸 뒤** 승인을 몰아서 합니다.

## 3. 검사
```
python3 scripts/validate_course.py --production
npm run verify
```
